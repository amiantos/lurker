...
const stub = {
  ...
  async upload(buffer, meta, secrets) {
    stub.capturedSecrets = secrets;
...
vi.mock('../services/uploadProviders/index.js', () => ({
  ...
  getProvider: () => stub,
  secretsForProvider: (_id, settings) => ({ token: settings['uploads.stub.token'] || '' }),
}));
...
describe('POST /api/uploads', () => {
  ...
  it('maps PROVIDER_CONFIG into 400', async () => {
    const err = new Error('missing config');
...
describe('DELETE /api/uploads/:id', () => {
  it('removes an owned row', async () => {
    ...
    const list = await agent.get('/api/uploads');
    expect(list.body.items.find((r) => r.id === upload.body.id)).toBeFalsy();
  });
