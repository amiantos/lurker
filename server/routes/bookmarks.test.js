...
describe('GET /api/bookmarks', () => {
  ...
  it('returns only the caller\'s bookmarks', async () => {
    const aliceMsg = chat(aliceNet.id, '#meta', 'someone', 'visible').id;
    const bobMsg = chat(bobNet.id, '#meta', 'someone', 'hidden').id;
    addBookmark(alice.id, aliceMsg);
    addBookmark(bob.id, bobMsg);

    const res = await aliceAgent.get('/api/bookmarks');
    expect(res.status).toBe(200);
    expect(res.body.items.map((r) => r.id)).toContain(aliceMsg);
    expect(res.body.items.map((r) => r.id)).not.toContain(bobMsg);
  });
