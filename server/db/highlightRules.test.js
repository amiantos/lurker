...
describe('upsertAutoNickRule', () => {
  it('creates an auto rule and attaches the network', () => {
    ...
    const links = db.prepare(`SELECT network_id FROM highlight_rule_networks WHERE rule_id = ?`).all(rule.id);
  ...
  it('attaches additional networks that share the same nick', () => {
    ...
    const links = db.prepare(`SELECT network_id FROM highlight_rule_networks WHERE rule_id = ? ORDER BY network_id`).all(rule.id);
  ...
  it('switching a network\'s nick detaches the old auto rule and sweeps it when orphaned', () => {
    ...
    const aliceLinks = db.prepare(`SELECT network_id FROM highlight_rule_networks WHERE rule_id = ?`).all(aliceRule.id);
    ...
    const newbieLinks = db.prepare(`SELECT network_id FROM highlight_rule_networks WHERE rule_id = ?`).all(newbieRule.id);
  ...
  it('null/empty nick returns null', () => {
    ...
    expect(mod.upsertAutoNickRule(user.id, net1.id, null)).toBeNull();
  });
});
