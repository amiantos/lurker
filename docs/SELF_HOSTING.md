...
**Away while the app is down.** A deploy is over before anyone notices, but a crash loop or a stopped app is not — and until it comes back you look present, so a message to you lands in that buffer and its sender hears nothing. After five minutes with no app attached (`LURKER_ENGINE_AWAY_AFTER_MS`, `0` turns it off) the engine marks each connection it holds away, and the app takes that back off when it returns. An away you set yourself is never touched.
