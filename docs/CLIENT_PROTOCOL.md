...
| `POST /`                                           | `{name*, host*, port, tls, nick*, username, realname, server_password, autoconnect, sasl_account, sasl_password, default_channel, connect_commands, trusted_certificates, generate_client_cert}` → `201 {network}`, connects immediately. `403` if host blocked by admin allowlist. `generate_client_cert` mints a CertFP pair BEFORE the first dial (`400` if `tls` is false) — attaching one afterwards misses the connect the user registers it from |
