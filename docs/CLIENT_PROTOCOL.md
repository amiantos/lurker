...
untagged networks and on optimistic self echoes. It is what a reaction replies
to: a row without one can't be reacted to.
...
**`reactions`** lists the IRCv3 reactions (`+draft/react`) standing on the row,
oldest first, as `[{ nick, value, self }]` — `self` is yours. Absent, not `[]`,
when there are none. Like `bookmarked`, keep a map of what you've seen: a row
...
| `sync-reactions`       | `messageIds:[…]` (≤5000)                                                                                                                                                                                                                              | Re-read the reactions on lines you already hold, after a resume: a `reaction` frame only reaches a connected socket, and `?since` ships only new rows. Answered to this socket as `reactions-sync` |
...
| `reaction`                                                                                                                       | `networkId, bufferId, target, messageId, nick, value, self, remove, toSelf, time`                                                                                                                                                                                                                                                                                                                           | A reaction was added (or, with `remove`, taken back) on the line `messageId`. `toSelf`: the line is yours, so it belongs in the activity feed. Patch it in if you hold that line; it never reopens a closed buffer |
| `reactions-sync`                                                                                                                 | `messageIds, reactions: { "<id>": [{ nick, value, self }] }`                                                                                                                                                                                                                                                                                                                                                | The answer to `sync-reactions`. Every id in `messageIds` is authoritative: one absent from `reactions` has none standing now                                                                                       |
...
| `GET /api/activity`           | The activity feed: highlights and other people's reactions to your lines, newest first. `?limit (≤200), beforeMessage, beforeReaction, networkId, q, nick (repeatable), target` → `{items, next}`. Each item has `kind: 'highlight'\|'reaction'`; `id` is always the line (the jump target). A reaction item adds `reactionId, value`, its `nick`/`time` are the reaction's, and `text` is your line's. Page by passing `next`'s fields back as-is; `next: null` = nothing older                                                                                                                                                                                                                                          |
